...
describe("remote control plane and outbound agent flow", () => {
  ...
  test("registers OAuth client, enrolls agent, enforces policy, updates policy, and revokes", async () => {
    ...
    try {
      ...
      // Policy denials are reported as JSON-RPC errors (HTTP 200) so MCP clients
      // surface a tool error instead of breaking the transport stream.
      expect(deniedShell.result).toBeUndefined();
      ...
      // Revoked-agent calls are reported as JSON-RPC errors (HTTP 200) too.
      expect(revokedResult.result).toBeUndefined();
