...
describe("SessionManager", () => {
  ...
  test("getActiveSessions excludes closing sessions", async () => {
    const result = await manager.openSession({
      host: "example.com",
      username: "demo",
      password: "secret",
    ...
    const closePromise = manager.closeSession(result.sessionId);

    const activeDuringClose = manager.getActiveSessions();
    expect(activeDuringClose.find((s) => s.sessionId === result.sessionId)).toBeUndefined();

    await expect(manager.closeSession(result.sessionId)).resolves.toBe(false);

    await closePromise;
    expect(manager.getActiveSessions()).toHaveLength(0);
  });

  test("getActiveSessions excludes expired sessions", async () => {
    const result = await manager.openSession({
      host: "example.com",
      username: "demo",
      password: "secret",
  ...
  test("cleanup does not evict session with active operation", async () => {
    const result = await manager.openSession({
      host: "example.com",
      username: "demo",
      password: "secret",
    ...
    manager.beginOperation(result.sessionId);
    ...
    manager.endOperation(result.sessionId);
  });

  test("cleanup evicts session after operation finishes and session expires", async () => {
    const result = await manager.openSession({
      host: "expire-after-op.example",
      username: "demo",
      password: "secret",
      auth: "password",
      ttlMs: 50,
  ...
  test("active operations guard against close during tunnel cleanup", async () => {
    const result = await manager.openSession({
      host: "tunnel-guard.example",
      username: "demo",
      password: "secret",
      auth: "password",
    });
